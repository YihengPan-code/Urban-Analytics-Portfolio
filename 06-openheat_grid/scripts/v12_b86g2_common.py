"""Shared helpers for the B8.6g2 feature-upgraded surrogate retest.

Inputs:
    configs/v12/systemb_b86g2_feature_retest.yaml and compact CSV inputs
    declared there.
Outputs:
    Helper functions only. Lane scripts write CSV/Markdown artifacts under
    outputs/v12_surrogate/b8_6g2_feature_retest and docs/v12.
Saved metrics:
    None directly; model, split, audit, diagnostic, and decision metrics are
    written by the lane scripts.
"""

from __future__ import annotations

import json
import math
import warnings
from dataclasses import dataclass
from pathlib import Path
from typing import Any, Iterable

import numpy as np
import pandas as pd
import yaml
from sklearn.base import clone
from sklearn.compose import ColumnTransformer
from sklearn.ensemble import (
    HistGradientBoostingClassifier,
    HistGradientBoostingRegressor,
    RandomForestClassifier,
    RandomForestRegressor,
)
from sklearn.exceptions import ConvergenceWarning
from sklearn.impute import SimpleImputer
from sklearn.linear_model import ElasticNet, LogisticRegression, Ridge
from sklearn.metrics import (
    accuracy_score,
    balanced_accuracy_score,
    mean_absolute_error,
    mean_squared_error,
    precision_score,
    r2_score,
    recall_score,
)
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import OneHotEncoder, StandardScaler


warnings.filterwarnings("ignore", category=FutureWarning, module="sklearn")
warnings.filterwarnings("ignore", category=ConvergenceWarning)
warnings.filterwarnings("ignore", message="y_pred contains classes not in y_true")
warnings.filterwarnings("ignore", message=".*physical cores.*")

PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "v12" / "systemb_b86g2_feature_retest.yaml"
CLAIM_BOUNDARY = (
    "SOLWEIG-derived compact Tmrt-delta feature-upgraded surrogate diagnostic only; "
    "not WBGT, risk, hazard_score, observed truth, causal feature importance, B9, "
    "AOI-wide prediction, Tmrt-to-WBGT conversion, or System A/B coupling."
)
CLASS_LABELS = ["neutral", "meaningful_cooling", "other_warming_or_weak_positive"]
WEAK_SPLITS = ["cell_group_holdout", "spatial_holdout", "typology_holdout"]
COMBO_COLS = ["feature_set", "neutral_threshold_c", "classifier", "regressor"]
FOLD_COLS = ["split_family", "split_name", "fold_id", "seed"]
FORBIDDEN_EXTENSIONS = {".tif", ".tiff", ".vrt", ".asc", ".img", ".nc", ".grib"}
FORBIDDEN_PATH_TOKENS = {"svfs.zip"}
FORBIDDEN_PREDICTOR_TOKENS = [
    "tmrt",
    "wbgt",
    "risk",
    "hazard",
    "score",
    "rank",
    "target",
    "output",
    "raster",
    "solweig",
    "qgis",
    "svfs",
    "tif",
    "tiff",
    "observed",
    "official",
    "truth",
    "method",
    "source",
    "status",
    "path",
    "notes",
    "vulnerability",
    "exposure",
    "elderly",
    "children",
    "demographic",
    "bus_stop",
    "mrt",
    "sport",
    "hawker",
    "eldercare",
    "preschool",
    "node",
    "population",
    "forecast",
    "archive",
]
FORBIDDEN_PREDICTOR_COLUMNS = {
    "cell_id",
    "forcing_day_id",
    "row_id",
    "base_tmrt_p90_c",
    "overhead_tmrt_p90_c",
    "within_slice_rank",
    "rank_direction",
    "feature_version",
}
ALLOWED_CATEGORICAL_FEATURES = {"typology_geometry_class"}
BASELINE_FEATURE_SET = "b86d_baseline_without_b86g"


@dataclass(frozen=True)
class Fold:
    """Deterministic validation fold metadata."""

    split_family: str
    split_name: str
    fold_id: str
    train_index: pd.Index
    test_index: pd.Index


def repo_path(path: str | Path) -> Path:
    """Resolve a project-relative path."""
    candidate = Path(path)
    return candidate if candidate.is_absolute() else PROJECT_ROOT / candidate


def assert_safe_compact_path(path: str | Path) -> None:
    """Reject raster-like or svfs.zip paths before any file IO."""
    candidate = repo_path(path)
    lower = str(candidate).lower().replace("\\", "/")
    if candidate.suffix.lower() in FORBIDDEN_EXTENSIONS or any(token in lower for token in FORBIDDEN_PATH_TOKENS):
        raise ValueError(f"B8.6g2 forbidden input/output path blocked: {path}")


def load_config(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    """Load the B8.6g2 YAML config."""
    with repo_path(config_path).open("r", encoding="utf-8") as handle:
        return yaml.safe_load(handle)


def output_path(config: dict[str, Any], key: str) -> Path:
    """Resolve one configured output path."""
    path = repo_path(config["outputs"][key])
    assert_safe_compact_path(path)
    return path


def ensure_output_dir(config: dict[str, Any]) -> Path:
    """Create the compact B8.6g2 output directory."""
    out_dir = output_path(config, "out_dir")
    out_dir.mkdir(parents=True, exist_ok=True)
    return out_dir


def read_csv(path: str | Path, **kwargs: Any) -> pd.DataFrame:
    """Read a UTF-8 compact CSV after the no-raster guard."""
    assert_safe_compact_path(path)
    return pd.read_csv(repo_path(path), **kwargs)


def write_csv(frame: pd.DataFrame, path: str | Path) -> None:
    """Write a UTF-8 CSV with parent creation."""
    destination = repo_path(path)
    assert_safe_compact_path(destination)
    destination.parent.mkdir(parents=True, exist_ok=True)
    frame.to_csv(destination, index=False, encoding="utf-8")


def write_text(text: str, path: str | Path) -> None:
    """Write UTF-8 text with parent creation."""
    destination = repo_path(path)
    assert_safe_compact_path(destination)
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(text, encoding="utf-8", newline="\n")


def guardrails_are_set(config: dict[str, Any]) -> None:
    """Validate explicit no-raster, no-QGIS/SOLWEIG, no-AOI, and no-B9 flags."""
    required = [
        "no_raster_io",
        "no_qgis_solweig",
        "no_aoi_prediction",
        "no_b9",
        "no_local_wbgt",
        "no_hazard_score",
        "no_risk_score",
        "no_system_a_b_coupling",
        "no_tmrt_to_wbgt_conversion",
        "no_observed_truth_claim",
        "no_causal_feature_importance_claim",
    ]
    missing = [key for key in required if config.get(key) is not True]
    if missing:
        raise ValueError(f"B8.6g2 guardrails are not explicit true values: {missing}")


def config_input_keys(config: dict[str, Any]) -> list[str]:
    """Return configured compact input keys."""
    return [
        key
        for key, value in config.items()
        if key.endswith("_path") and isinstance(value, str) and not key.startswith("outputs")
    ]


def input_inventory_frame(config: dict[str, Any]) -> pd.DataFrame:
    """Build compact input inventory without touching forbidden products."""
    rows: list[dict[str, Any]] = []
    for key in config_input_keys(config):
        path = config[key]
        assert_safe_compact_path(path)
        resolved = repo_path(path)
        row: dict[str, Any] = {
            "input_key": key,
            "path": path,
            "exists": resolved.exists(),
            "size_bytes": resolved.stat().st_size if resolved.exists() else None,
            "row_count": None,
            "column_count": None,
            "unique_cells": None,
            "forcing_day_count": None,
            "hour_count": None,
            "read_status": "MISSING",
            "allowed_for_b86g2": True,
            "claim_boundary": CLAIM_BOUNDARY,
        }
        if resolved.exists() and resolved.suffix.lower() == ".csv":
            frame = pd.read_csv(resolved)
            row["row_count"] = len(frame)
            row["column_count"] = len(frame.columns)
            row["unique_cells"] = int(frame["cell_id"].nunique()) if "cell_id" in frame.columns else None
            row["forcing_day_count"] = int(frame["forcing_day_id"].nunique()) if "forcing_day_id" in frame.columns else None
            row["hour_count"] = int(frame["hour_sgt"].nunique()) if "hour_sgt" in frame.columns else None
            row["read_status"] = "READ_OK"
        elif resolved.exists():
            row["read_status"] = "EXISTS_NOT_CSV_NOT_READ"
        rows.append(row)
    return pd.DataFrame(rows)


def now_stamp() -> str:
    """Return a local timestamp string."""
    return pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")


def parse_bool(value: Any) -> bool:
    """Parse common CSV boolean-like values."""
    if isinstance(value, bool):
        return value
    return str(value).strip().lower() in {"true", "1", "yes", "y"}


def parse_pipe_list(value: Any) -> list[str]:
    """Parse pipe-delimited feature lists."""
    if value is None or (isinstance(value, float) and math.isnan(value)):
        return []
    if isinstance(value, list):
        return [str(item) for item in value if str(item)]
    return [part for part in str(value).split("|") if part]


def forbidden_predictor(column: str) -> bool:
    """Return True if a column violates the B8.6g2 predictor contract."""
    name = column.lower()
    if name in ALLOWED_CATEGORICAL_FEATURES or name == "hour_sgt":
        return False
    if name in FORBIDDEN_PREDICTOR_COLUMNS:
        return True
    return any(token in name for token in FORBIDDEN_PREDICTOR_TOKENS)


def b86g_feature_schema(config: dict[str, Any]) -> pd.DataFrame:
    """Read the B8.6g feature schema."""
    return read_csv(config["b86g_feature_schema_path"])


def non_null_numeric_columns(dataset: pd.DataFrame, columns: Iterable[str]) -> list[str]:
    """Return columns coercible to numeric with at least one non-null value."""
    out: list[str] = []
    for column in columns:
        if column not in dataset.columns or forbidden_predictor(column):
            continue
        converted = pd.to_numeric(dataset[column], errors="coerce")
        if int(converted.notna().sum()) > 0:
            out.append(column)
    return out


def feature_columns_for_set(registry: pd.DataFrame, dataset: pd.DataFrame, feature_set: str) -> list[str]:
    """Return available, non-forbidden feature columns for a named feature set."""
    row = registry.loc[registry["feature_set"].astype(str).eq(str(feature_set))]
    if row.empty:
        return []
    columns = parse_pipe_list(row.iloc[0]["feature_columns"])
    return [column for column in columns if column in dataset.columns and not forbidden_predictor(column)]


def build_feature_registry(dataset: pd.DataFrame, schema: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    """Create the B8.6g2 feature-set registry."""
    schema = schema.copy()
    schema["proxy_flag_bool"] = schema["proxy_flag"].map(parse_bool)
    feature_names = [str(name) for name in schema["feature_name"] if str(name) in dataset.columns]
    numeric_schema = schema.loc[schema["type"].astype(str).str.lower().isin(["float", "integer", "int"])]
    categorical_schema = schema.loc[schema["type"].astype(str).str.lower().eq("category")]
    safe_numeric = non_null_numeric_columns(dataset, numeric_schema["feature_name"].astype(str))
    safe_categorical = [
        column
        for column in categorical_schema["feature_name"].astype(str)
        if column in dataset.columns and column in ALLOWED_CATEGORICAL_FEATURES and int(dataset[column].notna().sum()) > 0
    ]
    proxy_numeric = [
        column
        for column in safe_numeric
        if bool(schema.set_index("feature_name").loc[column, "proxy_flag_bool"]) or column.endswith("_proxy")
    ]
    vector_compact = [
        column
        for column in safe_numeric
        if "compact_vector_derived" in str(schema.set_index("feature_name").loc[column, "source_type"]).lower()
        and not bool(schema.set_index("feature_name").loc[column, "proxy_flag_bool"])
    ]
    high_priority_families = {
        "pedestrian-accessible shaded fraction",
        "overhead geometry shape descriptors",
        "sunlit-hot-pocket area fraction",
        "tree/building shadow interaction",
        "canyon orientation / height roughness",
    }
    high_priority = [
        column
        for column in safe_numeric
        if str(schema.set_index("feature_name").loc[column, "feature_family"]) in high_priority_families
    ]
    baseline = [column for column in config["baseline_feature_columns"] if column in dataset.columns and not forbidden_predictor(column)]

    def with_hour(columns: list[str]) -> list[str]:
        merged = list(dict.fromkeys(columns + ["hour_sgt"]))
        return [column for column in merged if column in dataset.columns and not forbidden_predictor(column)]

    definitions = {
        BASELINE_FEATURE_SET: {
            "columns": baseline,
            "definition": "Comparable B8.6d compact baseline features rebuilt from the N150 sampling matrix; excludes B8.6g features.",
        },
        "b86g_proxy_features_only": {
            "columns": with_hour(proxy_numeric),
            "definition": "Numeric B8.6g proxy or proxy-derived features plus hour_sgt.",
        },
        "b86g_vector_derived_compact_only": {
            "columns": with_hour(vector_compact),
            "definition": "Compact vector-derived overhead geometry features plus hour_sgt.",
        },
        "b86g_proxy_plus_vector_compact": {
            "columns": with_hour(proxy_numeric + vector_compact),
            "definition": "Union of B8.6g proxy and compact vector-derived numeric features plus hour_sgt.",
        },
        "b86g_no_status_columns": {
            "columns": with_hour(safe_numeric + safe_categorical),
            "definition": "All safe B8.6g non-status/non-method/non-source features, including allowed categorical typology geometry.",
        },
        "b86g_high_priority_only": {
            "columns": with_hour(high_priority),
            "definition": "Numeric B8.6g features from high-priority feature families plus hour_sgt.",
        },
        "b86g_all_safe_numeric": {
            "columns": with_hour(safe_numeric),
            "definition": "All safe numeric B8.6g features except status/method/source metadata plus hour_sgt.",
        },
    }
    rows: list[dict[str, Any]] = []
    for feature_set in config["feature_sets_to_test"]:
        columns = [column for column in definitions[feature_set]["columns"] if column in dataset.columns]
        schema_index = schema.set_index("feature_name")
        b86g_columns = [column for column in columns if column in feature_names]
        proxy_count = sum(bool(schema_index.loc[column, "proxy_flag_bool"]) for column in b86g_columns)
        vector_count = sum("vector" in str(schema_index.loc[column, "source_type"]).lower() for column in b86g_columns)
        categorical_count = sum(column in safe_categorical for column in columns)
        rows.append(
            {
                "feature_set": feature_set,
                "feature_count": len(columns),
                "numeric_feature_count": len(columns) - categorical_count,
                "categorical_feature_count": categorical_count,
                "proxy_feature_count": int(proxy_count),
                "vector_or_vector_compact_feature_count": int(vector_count),
                "uses_hour_sgt": "hour_sgt" in columns,
                "coordinate_dependent": False,
                "target_derived": False,
                "feature_columns": "|".join(columns),
                "feature_definition": definitions[feature_set]["definition"],
                "status": "AVAILABLE" if columns else "BLOCKED_NO_FEATURES",
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
    return pd.DataFrame(rows)


def coerce_feature_frame(dataset: pd.DataFrame, features: list[str]) -> tuple[pd.DataFrame, list[str], list[str]]:
    """Split selected features into numeric and categorical frame columns."""
    frame = dataset[features].copy()
    numeric: list[str] = []
    categorical: list[str] = []
    for column in features:
        converted = pd.to_numeric(frame[column], errors="coerce")
        source_non_null = int(frame[column].notna().sum())
        if source_non_null == 0 or converted.notna().sum() >= max(1, int(0.95 * source_non_null)):
            frame[column] = converted
            numeric.append(column)
        else:
            frame[column] = frame[column].astype("string")
            categorical.append(column)
    return frame, numeric, categorical


def one_hot_encoder() -> OneHotEncoder:
    """Create a scikit-learn version-compatible one-hot encoder."""
    try:
        return OneHotEncoder(handle_unknown="ignore", sparse_output=False)
    except TypeError:
        return OneHotEncoder(handle_unknown="ignore", sparse=False)


def make_preprocessor(model_name: str, numeric: list[str], categorical: list[str]) -> ColumnTransformer:
    """Build preprocessing for compact numeric/categorical features."""
    transformers: list[tuple[str, Pipeline, list[str]]] = []
    if numeric:
        steps: list[tuple[str, Any]] = [("imputer", SimpleImputer(strategy="median"))]
        if model_name in {"ridge", "elasticnet", "logistic_regression"}:
            steps.append(("scaler", StandardScaler()))
        transformers.append(("num", Pipeline(steps), numeric))
    if categorical:
        transformers.append(
            (
                "cat",
                Pipeline([("imputer", SimpleImputer(strategy="most_frequent")), ("onehot", one_hot_encoder())]),
                categorical,
            )
        )
    return ColumnTransformer(transformers=transformers, remainder="drop", sparse_threshold=0.0)


def make_pipeline(model_name: str, estimator: Any, numeric: list[str], categorical: list[str]) -> Pipeline:
    """Create a preprocessing + estimator pipeline."""
    return Pipeline([("prep", make_preprocessor(model_name, numeric, categorical)), ("model", clone(estimator))])


def classifier_models(config: dict[str, Any], seed: int | None = None) -> dict[str, Any]:
    """Configured B8.6g2 stage-1 classifiers."""
    seed_value = int(config["random_seed"] if seed is None else seed)
    specs = config["models"]
    return {
        "logistic_regression": LogisticRegression(
            max_iter=int(specs["logistic_regression"]["max_iter"]),
            solver="liblinear",
            multi_class="ovr",
        ),
        "random_forest_classifier": RandomForestClassifier(
            n_estimators=int(specs["random_forest_classifier"]["n_estimators"]),
            max_depth=int(specs["random_forest_classifier"]["max_depth"]),
            min_samples_leaf=int(specs["random_forest_classifier"]["min_samples_leaf"]),
            random_state=seed_value,
            n_jobs=-1,
            class_weight="balanced_subsample",
        ),
        "hist_gradient_boosting_classifier": HistGradientBoostingClassifier(
            max_iter=int(specs["hist_gradient_boosting_classifier"]["max_iter"]),
            learning_rate=float(specs["hist_gradient_boosting_classifier"]["learning_rate"]),
            max_leaf_nodes=int(specs["hist_gradient_boosting_classifier"]["max_leaf_nodes"]),
            random_state=seed_value,
        ),
    }


def regressor_models(config: dict[str, Any], seed: int | None = None) -> dict[str, Any]:
    """Configured B8.6g2 regressors."""
    seed_value = int(config["random_seed"] if seed is None else seed)
    specs = config["models"]
    return {
        "ridge": Ridge(alpha=float(specs["ridge"]["alpha"])),
        "elasticnet": ElasticNet(
            alpha=float(specs["elasticnet"]["alpha"]),
            l1_ratio=float(specs["elasticnet"]["l1_ratio"]),
            random_state=seed_value,
            max_iter=10000,
        ),
        "random_forest_regressor": RandomForestRegressor(
            n_estimators=int(specs["random_forest_regressor"]["n_estimators"]),
            max_depth=int(specs["random_forest_regressor"]["max_depth"]),
            min_samples_leaf=int(specs["random_forest_regressor"]["min_samples_leaf"]),
            random_state=seed_value,
            n_jobs=-1,
        ),
        "hist_gradient_boosting_regressor": HistGradientBoostingRegressor(
            max_iter=int(specs["hist_gradient_boosting_regressor"]["max_iter"]),
            learning_rate=float(specs["hist_gradient_boosting_regressor"]["learning_rate"]),
            max_leaf_nodes=int(specs["hist_gradient_boosting_regressor"]["max_leaf_nodes"]),
            random_state=seed_value,
        ),
    }


def neutral_class(values: pd.Series | np.ndarray, threshold: float) -> np.ndarray:
    """Map primary deltas to neutral/cooling/other classes."""
    numeric = np.asarray(values, dtype=float)
    labels = np.full(numeric.shape, "other_warming_or_weak_positive", dtype=object)
    labels[np.abs(numeric) <= threshold] = "neutral"
    labels[numeric < -threshold] = "meaningful_cooling"
    return labels


def finite_corr(y_true: np.ndarray, y_pred: np.ndarray, method: str) -> float:
    """Compute finite Pearson or Spearman correlation."""
    frame = pd.DataFrame({"true": y_true, "pred": y_pred}).dropna()
    if len(frame) < 2 or frame["true"].nunique() <= 1 or frame["pred"].nunique() <= 1:
        return float("nan")
    left = frame["true"].to_numpy(dtype=float)
    right = frame["pred"].to_numpy(dtype=float)
    if method == "spearman":
        left = pd.Series(left).rank(method="average").to_numpy(dtype=float)
        right = pd.Series(right).rank(method="average").to_numpy(dtype=float)
    left = left - float(np.mean(left))
    right = right - float(np.mean(right))
    denom = math.sqrt(float(np.sum(left * left)) * float(np.sum(right * right)))
    return float(np.sum(left * right) / denom) if denom else float("nan")


def cell_average_frame(test: pd.DataFrame, y_true: np.ndarray, y_pred: np.ndarray) -> pd.DataFrame:
    """Return per-cell mean true and predicted delta values."""
    frame = test[["cell_id"]].copy()
    frame["y_true"] = y_true
    frame["y_pred"] = y_pred
    return frame.groupby("cell_id", as_index=False)[["y_true", "y_pred"]].mean(numeric_only=True)


def top_overlap(test: pd.DataFrame, y_true: np.ndarray, y_pred: np.ndarray, k: int) -> float:
    """Compute overlap among the most negative true/predicted cell deltas."""
    by_cell = cell_average_frame(test, y_true, y_pred)
    if by_cell.empty:
        return float("nan")
    k = min(max(1, k), len(by_cell))
    true_top = set(by_cell.nsmallest(k, "y_true")["cell_id"].astype(str))
    pred_top = set(by_cell.nsmallest(k, "y_pred")["cell_id"].astype(str))
    return float(len(true_top & pred_top) / k)


def top_fraction_overlap(test: pd.DataFrame, y_true: np.ndarray, y_pred: np.ndarray, fraction: float) -> float:
    """Compute top cooling overlap at a cell fraction."""
    return top_overlap(test, y_true, y_pred, max(1, int(math.ceil(fraction * int(test["cell_id"].nunique())))))


def regression_metrics(test: pd.DataFrame, y_true: np.ndarray, y_pred: np.ndarray, threshold: float) -> dict[str, float]:
    """Compute compact surrogate regression and ranking metrics."""
    if len(y_true) == 0:
        return {
            "MAE": float("nan"),
            "RMSE": float("nan"),
            "R2": float("nan"),
            "Spearman": float("nan"),
            "Pearson": float("nan"),
            "Spearman_observed_vs_predicted": float("nan"),
            "Pearson_observed_vs_predicted": float("nan"),
            "bias": float("nan"),
            "p90_abs_error": float("nan"),
            "sign_accuracy": float("nan"),
            "top5_overlap": float("nan"),
            "top10pct_overlap": float("nan"),
            "top20pct_overlap": float("nan"),
        }
    err = np.asarray(y_pred, dtype=float) - np.asarray(y_true, dtype=float)
    sign_true = np.asarray(y_true, dtype=float) < -threshold
    sign_pred = np.asarray(y_pred, dtype=float) < -threshold
    spearman = finite_corr(y_true, y_pred, "spearman")
    pearson = finite_corr(y_true, y_pred, "pearson")
    return {
        "MAE": float(mean_absolute_error(y_true, y_pred)),
        "RMSE": float(math.sqrt(mean_squared_error(y_true, y_pred))),
        "R2": float(r2_score(y_true, y_pred)) if len(np.unique(y_true)) > 1 else float("nan"),
        "Spearman": spearman,
        "Pearson": pearson,
        "Spearman_observed_vs_predicted": spearman,
        "Pearson_observed_vs_predicted": pearson,
        "bias": float(np.mean(err)),
        "p90_abs_error": float(np.quantile(np.abs(err), 0.90)),
        "sign_accuracy": float(np.mean(sign_true == sign_pred)),
        "top5_overlap": top_overlap(test, y_true, y_pred, 5),
        "top10pct_overlap": top_fraction_overlap(test, y_true, y_pred, 0.10),
        "top20pct_overlap": top_fraction_overlap(test, y_true, y_pred, 0.20),
    }


def classification_metrics(y_true_class: np.ndarray, y_pred_class: np.ndarray) -> dict[str, float | str]:
    """Compute neutral-boundary classification metrics."""
    present_labels = [label for label in CLASS_LABELS if label in set(y_true_class) or label in set(y_pred_class)]
    out: dict[str, float | str] = {
        "accuracy": float(accuracy_score(y_true_class, y_pred_class)),
        "balanced_accuracy": float(balanced_accuracy_score(y_true_class, y_pred_class)),
    }
    for label in ["neutral", "meaningful_cooling"]:
        out[f"precision_{label}"] = float(
            precision_score(y_true_class, y_pred_class, labels=[label], average="macro", zero_division=0)
        )
        out[f"recall_{label}"] = float(
            recall_score(y_true_class, y_pred_class, labels=[label], average="macro", zero_division=0)
        )
    true_neutral = y_true_class == "neutral"
    true_cooling = y_true_class == "meaningful_cooling"
    out["neutral_accuracy"] = (
        float(np.mean(y_pred_class[true_neutral] == "neutral")) if int(true_neutral.sum()) else float("nan")
    )
    out["false_promotion_rate"] = (
        float(np.mean(y_pred_class[true_neutral] == "meaningful_cooling")) if int(true_neutral.sum()) else float("nan")
    )
    out["false_neutral_rate"] = (
        float(np.mean(y_pred_class[true_cooling] == "neutral")) if int(true_cooling.sum()) else float("nan")
    )
    out["true_neutral_count"] = int(true_neutral.sum())
    out["true_meaningful_cooling_count"] = int(true_cooling.sum())
    out["true_other_warming_or_weak_positive_count"] = int((y_true_class == "other_warming_or_weak_positive").sum())
    confusion = {
        true_label: {
            pred_label: int(((y_true_class == true_label) & (y_pred_class == pred_label)).sum())
            for pred_label in CLASS_LABELS
        }
        for true_label in CLASS_LABELS
    }
    out["confusion_matrix_json"] = json.dumps(confusion, sort_keys=True)
    out["present_labels"] = "|".join(present_labels)
    return out


def role_error_and_rank(test: pd.DataFrame, y_true: np.ndarray, y_pred: np.ndarray, cells: Iterable[str]) -> tuple[float, float]:
    """Compute diagnostic cell MAE and mean absolute rank error."""
    by_cell = cell_average_frame(test, y_true, y_pred)
    if by_cell.empty:
        return float("nan"), float("nan")
    by_cell["true_rank"] = by_cell["y_true"].rank(method="min", ascending=True)
    by_cell["pred_rank"] = by_cell["y_pred"].rank(method="min", ascending=True)
    subset = by_cell.loc[by_cell["cell_id"].astype(str).isin(set(map(str, cells)))].copy()
    if subset.empty:
        return float("nan"), float("nan")
    mae = float((subset["y_pred"] - subset["y_true"]).abs().mean())
    rank_error = float((subset["pred_rank"] - subset["true_rank"]).abs().mean())
    return mae, rank_error


def validation_folds(dataset: pd.DataFrame, config: dict[str, Any]) -> list[Fold]:
    """Create deterministic non-random validation folds."""
    folds: list[Fold] = []
    families = set(config["split_families"])
    if "forcing_day_holdout" in families:
        for idx, forcing_day in enumerate(sorted(dataset["forcing_day_id"].astype(str).unique()), start=1):
            mask = dataset["forcing_day_id"].astype(str).eq(str(forcing_day))
            folds.append(Fold("forcing_day_holdout", f"holdout_{forcing_day}", str(idx), dataset.index[~mask], dataset.index[mask]))

    if "cell_group_holdout" in families:
        cells = np.array(sorted(dataset["cell_id"].astype(str).unique()))
        rng = np.random.default_rng(int(config["random_seed"]))
        rng.shuffle(cells)
        for idx, test_cells in enumerate(np.array_split(cells, int(config["validation"]["cell_group_folds"])), start=1):
            mask = dataset["cell_id"].astype(str).isin(set(test_cells.tolist()))
            folds.append(Fold("cell_group_holdout", "cell_group_5fold", str(idx), dataset.index[~mask], dataset.index[mask]))

    if "spatial_holdout" in families:
        cells = dataset.drop_duplicates("cell_id")[["cell_id", "centroid_x", "centroid_y"]].copy()
        cells["centroid_x"] = pd.to_numeric(cells["centroid_x"], errors="coerce")
        cells["centroid_y"] = pd.to_numeric(cells["centroid_y"], errors="coerce")
        x_mid = float(cells["centroid_x"].median())
        y_mid = float(cells["centroid_y"].median())
        cells["spatial_block"] = np.where(cells["centroid_x"] <= x_mid, "west", "east") + "_" + np.where(
            cells["centroid_y"] <= y_mid,
            "south",
            "north",
        )
        for idx, block in enumerate(sorted(cells["spatial_block"].unique()), start=1):
            test_cells = set(cells.loc[cells["spatial_block"].astype(str).eq(str(block)), "cell_id"].astype(str))
            mask = dataset["cell_id"].astype(str).isin(test_cells)
            folds.append(Fold("spatial_holdout", f"spatial_{block}", str(idx), dataset.index[~mask], dataset.index[mask]))

    if "typology_holdout" in families:
        min_test = int(config["validation"]["typology_min_test_cells"])
        min_train = int(config["validation"]["typology_min_train_cells"])
        typology_cells = dataset.drop_duplicates("cell_id")[["cell_id", "typology_label"]].dropna()
        fold_id = 1
        for typology in sorted(typology_cells["typology_label"].astype(str).unique()):
            test_cells = set(typology_cells.loc[typology_cells["typology_label"].astype(str).eq(typology), "cell_id"].astype(str))
            mask = dataset["cell_id"].astype(str).isin(test_cells)
            if len(test_cells) >= min_test and int(dataset.loc[~mask, "cell_id"].nunique()) >= min_train:
                folds.append(Fold("typology_holdout", f"typology_{typology}", str(fold_id), dataset.index[~mask], dataset.index[mask]))
            fold_id += 1

    if "hour_holdout" in families:
        hours = sorted(pd.to_numeric(dataset["hour_sgt"], errors="coerce").dropna().astype(int).unique())
        for idx, hour in enumerate(hours, start=1):
            mask = pd.to_numeric(dataset["hour_sgt"], errors="coerce").eq(hour)
            folds.append(Fold("hour_holdout", f"leave_h{hour}_out", str(idx), dataset.index[~mask], dataset.index[mask]))

    if bool(config["validation"].get("random_split_diagnostic", False)):
        rng = np.random.default_rng(int(config["random_seed"]))
        mask = pd.Series(rng.random(len(dataset)) < float(config["validation"]["random_test_fraction"]), index=dataset.index)
        folds.append(Fold("random_split", "random_row_diagnostic_not_main", "1", dataset.index[~mask], dataset.index[mask]))
    return folds


def fold_inventory(dataset: pd.DataFrame, folds: list[Fold]) -> pd.DataFrame:
    """Create fold-level row/cell inventory."""
    rows: list[dict[str, Any]] = []
    for fold in folds:
        train = dataset.loc[fold.train_index]
        test = dataset.loc[fold.test_index]
        rows.append(
            {
                "split_family": fold.split_family,
                "split_name": fold.split_name,
                "fold_id": fold.fold_id,
                "n_train": len(train),
                "n_test": len(test),
                "n_train_cells": int(train["cell_id"].nunique()),
                "n_test_cells": int(test["cell_id"].nunique()),
                "n_train_forcing_days": int(train["forcing_day_id"].nunique()),
                "n_test_forcing_days": int(test["forcing_day_id"].nunique()),
                "n_train_hours": int(train["hour_sgt"].nunique()),
                "n_test_hours": int(test["hour_sgt"].nunique()),
                "random_split_main_evidence": False,
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
    return pd.DataFrame(rows)


def metric_group_summary(frame: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    """Aggregate fold-level model metrics by mean."""
    metric_cols = [
        "MAE",
        "RMSE",
        "R2",
        "Spearman",
        "Pearson",
        "bias",
        "p90_abs_error",
        "sign_accuracy",
        "top5_overlap",
        "top10pct_overlap",
        "top20pct_overlap",
        "accuracy",
        "balanced_accuracy",
        "neutral_accuracy",
        "precision_neutral",
        "recall_neutral",
        "precision_meaningful_cooling",
        "recall_meaningful_cooling",
        "false_promotion_rate",
        "false_neutral_rate",
        "anchor_MAE",
        "anchor_mean_rank_error",
        "unstable_MAE",
        "unstable_mean_rank_error",
    ]
    available = [column for column in metric_cols if column in frame.columns]
    if frame.empty:
        return pd.DataFrame(columns=group_cols + ["n_folds", *available])
    grouped = frame.groupby(group_cols, dropna=False)
    out = grouped[available].mean(numeric_only=True).reset_index()
    out.insert(len(group_cols), "n_folds", grouped.size().to_numpy())
    out["claim_boundary"] = CLAIM_BOUNDARY
    return out


def selected_two_stage_combo(metrics: pd.DataFrame, config: dict[str, Any]) -> dict[str, Any]:
    """Select the primary feature-upgraded two-stage workflow."""
    ok = metrics.loc[metrics["status"].astype(str).eq("OK")].copy()
    ok = ok.loc[pd.to_numeric(ok["neutral_threshold_c"], errors="coerce").eq(float(config["neutral_threshold_c"]))]
    support = ok.loc[ok["split_family"].isin(WEAK_SPLITS)].copy()
    if support.empty:
        support = ok.copy()
    if support.empty:
        return {"feature_set": "", "neutral_threshold_c": float(config["neutral_threshold_c"]), "classifier": "", "regressor": ""}
    grouped = support.groupby(COMBO_COLS, dropna=False).agg(
        weak_spearman=("Spearman", "mean"),
        weak_top10=("top10pct_overlap", "mean"),
        weak_false_promotion=("false_promotion_rate", "mean"),
        weak_mae=("MAE", "mean"),
        weak_anchor_mae=("anchor_MAE", "mean"),
    ).reset_index()
    spatial = support.loc[support["split_family"].eq("spatial_holdout")].groupby(COMBO_COLS, dropna=False).agg(
        spatial_spearman=("Spearman", "mean"),
        spatial_top10=("top10pct_overlap", "mean"),
    ).reset_index()
    grouped = grouped.merge(spatial, on=COMBO_COLS, how="left")
    grouped["score"] = (
        grouped["spatial_spearman"].fillna(grouped["weak_spearman"]).fillna(-1.0) * 0.35
        + grouped["spatial_top10"].fillna(0.0) * 0.20
        + grouped["weak_spearman"].fillna(-1.0) * 0.20
        + grouped["weak_top10"].fillna(0.0) * 0.15
        - grouped["weak_false_promotion"].fillna(1.0) * 0.08
        - grouped["weak_mae"].fillna(1.0) * 0.02
    )
    row = grouped.sort_values(["score", "spatial_spearman", "weak_top10"], ascending=False).iloc[0]
    return row.to_dict()


def markdown_table(frame: pd.DataFrame, columns: list[str], max_rows: int = 12) -> str:
    """Format a compact Markdown table."""
    if frame.empty:
        return "_No rows._"
    view = frame.loc[:, [column for column in columns if column in frame.columns]].head(max_rows).copy()
    for column in view.select_dtypes(include=[float]).columns:
        view[column] = view[column].map(lambda value: "" if pd.isna(value) else f"{value:.4f}")
    return view.to_markdown(index=False)


def safe_float(value: Any, default: float = float("nan")) -> float:
    """Convert a scalar to float safely."""
    try:
        return float(value)
    except (TypeError, ValueError):
        return default

