...
def maybe_add_satellite_basemap(ax, cfg: dict):
    ...
    try:
        ...
        # contextily expects zoom to be an int or "auto"; passing None
        # triggers TypeError in some versions.  Omit zoom when config uses
        # null/auto so contextily can choose automatically.
        zoom = bcfg.get("zoom", "auto")
        if zoom not in (None, "auto", "AUTO", "Auto"):
            kwargs["zoom"] = zoom
