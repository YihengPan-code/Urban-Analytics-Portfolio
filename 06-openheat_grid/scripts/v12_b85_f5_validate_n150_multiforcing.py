"""Validate the B8.5-F5 human-controlled N150 / 3000-run package.
...
conversion, stage, or commit. It only checks metadata, local run-log rows, and
local file existence/size for expected outputs.
"""
...
def validate_manifest_scope(config: dict[str, Any], rows: Sequence[dict[str, str]]) -> list[str]:
    ...
    required_fields = set(manifest_fieldnames())
    observed_fields = set(rows[0].keys()) if rows else set()
    missing_fields = sorted(required_fields - observed_fields)
    if missing_fields:
        notes.append("missing_manifest_fields=" + ",".join(missing_fields))
    ...
    for row in rows:
        ...
        if group.startswith(("b85_f3a", "b85_f3b", "b85_f3c")):
            notes.append(f"prior_lane_output_group_reused={row.get('run_id')}")
        if "v12_n150_tiles" in tmrt_path.as_posix():
            notes.append(f"old_single_forcing_output_folder_reused={row.get('run_id')}")
    ...
    return [
        {
            ...
            "notes": f"No local run log found at {log_path.as_posix()}; human N150 multi-forcing runset has not been executed yet.",
        }
...
def validate_executed_rows(config: dict[str, Any], manifest_rows: Sequence[dict[str, str]], log_rows: Sequence[dict[str, str]]) -> tuple[list[dict[str, str]], bool, bool]:
...
def compact_postrun_status(rows: Sequence[dict[str, str]], decision: str) -> str:
    ...
    if decision == READY_FOR_HUMAN_N150_MULTIFORCING:
        return NOT_RUN_YET
...
def validate(config_path: Path) -> int:
    """Validate the prepared N150 runset and optional human run log."""
    ...
    if forbidden_hits:
        ...
        reason = f"Pre-execution ready count {ready_count}/{expected_runs}; human N150 multi-forcing runset remains blocked."
        postrun_rows = blocked_rows(manifest_rows, reason, BLOCKED_PRECHECK)
        decision = BLOCKED_PRECHECK
        notes = reason
        exit_code = 2
        ...
        if all_log_success and all_outputs_present:
            ...
            decision = N150_MULTIFORCING_EXECUTED_PARTIAL
    ...
    raster_status = NOT_RUN_YET if decision != N150_MULTIFORCING_EXECUTED_PASS else "PENDING_RASTER_QA"
    ...
    write_status(repo_path(outputs["status"]), config, decision, len(manifest_rows), unique_cell_count, ready_count, postrun_status, raster_status, label_status, stability_status, notes)
    ...
    print(f"Raster QA status: {raster_status}")
    print(f"Label merge status: {label_status}")
...
def main() -> int:
    """Parse CLI arguments and validate the F5 N150 runset."""
    parser = argparse.ArgumentParser(
        description=(
            "Validate the B8.5-F5 N150 / 3000-run package and optional local-only "
