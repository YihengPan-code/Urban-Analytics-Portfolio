...
Outputs:
  ...
  - optionally outputs/v12_solweig_n24_execution/n24_modifier_targets_provisional.csv

Saved metrics:
  - focus-cell valid-pixel count, mean, p50/p75/p90/p95/max Tmrt.
  - percent of focus-cell pixels with Tmrt >= 40/45/50/55 C.
  - paired overhead_as_canopy minus base deltas where both scenarios exist.
...
def resolve_tmrt_path(row: pd.Series) -> Path | None:
    direct = repo_path(str(row.get("tmrt_output_path", "")))
    if direct.exists():
        return direct
    out_dir = repo_path(str(row.get("output_dir", "")))
...
def write_report(out_dir: Path, summary: pd.DataFrame, delta: pd.DataFrame) -> None:
    lines = ["# Sprint B3 N24 Tmrt Aggregation Report", ""]
    lines.append(f"- focus summary rows: `{len(summary)}`")
    lines.append(f"- expected full rows: `240`")
    lines.append(f"- base vs overhead delta rows: `{len(delta)}`")
    if len(summary):
        lines.append(f"- valid pixel min/median/max: `{summary['valid_pixel_count'].min()}` / `{summary['valid_pixel_count'].median()}` / `{summary['valid_pixel_count'].max()}`")
        lines.append(f"- threshold-area metrics available: `{all(c in summary.columns for c in ['pct_pixels_tmrt_ge_40', 'pct_pixels_tmrt_ge_45', 'pct_pixels_tmrt_ge_50', 'pct_pixels_tmrt_ge_55'])}`")
    lines.append("")
    lines.append("These are SOLWEIG-derived Tmrt summaries only. They are not local WBGT, hazard_score, risk_score, or observed truth.")
    (out_dir / "n24_tmrt_aggregation_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def maybe_write_provisional_modifier(out_dir: Path, summary: pd.DataFrame) -> None:
    spec = PROJECT_ROOT / "docs/v12/OpenHeat_modifier_target_spec_CN.md"
    if not spec.exists() or summary.empty:
        return
    df = summary.copy()
    ref = (
        df.groupby(["hour", "scenario"], as_index=False)["tmrt_p90_c"]
        .median()
        .rename(columns={"tmrt_p90_c": "tmrt_ref_p90_c"})
    )
    m = df.merge(ref, on=["hour", "scenario"], how="left")
    m["reference_definition"] = "provisional_n24_batch_hour_scenario_median_reference"
    m["delta_tmrt_p90_c"] = m["tmrt_p90_c"] - m["tmrt_ref_p90_c"]
    m["m_rad_pct"] = m.groupby(["hour", "scenario"])["delta_tmrt_p90_c"].rank(pct=True, method="average")
    m.to_csv(out_dir / "n24_modifier_targets_provisional.csv", index=False)


def main() -> None:
    parser = argparse.ArgumentParser(description="Aggregate N24 SOLWEIG Tmrt rasters into focus-cell summaries and paired deltas.")
    ...
    if not log_path.exists():
        raise FileNotFoundError(f"SOLWEIG run log not found: {log_path}")
    ...
    geom_by_cell = {
        ...
        for _, row in grid[grid["cell_id"].isin(usable["cell_id"].astype(str).unique())].iterrows()
    }
    ...
    for _, row in usable.iterrows():
        ...
        if cell_id not in geom_by_cell:
            continue
    ...
    summary = pd.DataFrame(rows).sort_values(["cell_id", "scenario", "hour"]) if rows else pd.DataFrame()
    summary.to_csv(out_dir / "n24_focus_tmrt_summary.csv", index=False)
    ...
    if summary.empty:
        ...
        delta = b.merge(o, on=["cell_id", "hour"], how="inner", validate="one_to_one")
        rename_map = {
        ...
        for m, delta_col in rename_map.items():
            delta[delta_col] = delta[f"{m}_overhead_as_canopy"] - delta[f"{m}_base"]
    delta.to_csv(out_dir / "n24_base_vs_overhead_delta.csv", index=False)
    maybe_write_provisional_modifier(out_dir, summary)
    write_report(out_dir, summary, delta)
    print(f"[OK] wrote {out_dir / 'n24_focus_tmrt_summary.csv'} rows={len(summary)}")
    print(f"[OK] wrote {out_dir / 'n24_base_vs_overhead_delta.csv'} rows={len(delta)}")
