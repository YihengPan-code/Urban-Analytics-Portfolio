"""Human-controlled B8.5-F3a QGIS/SOLWEIG micro-batch runner.
...
Default behavior:
    ...
    a human must copy this script to C:/OpenHeat-local/solweig/b85_f3a_microbatch,
    manually change DRY_RUN to False in that local-only copy, confirm the
    4-row manifest, and run inside QGIS Desktop with UMEP available.
"""
...
CONFIG_PATH = PROJECT_ROOT / "configs/v12/systemb_b85_f3a_microbatch_execution.yaml"
...
def parse_scalar(value: str) -> Any:
    """Parse the small YAML scalar subset used by the F3a config."""
...
def read_simple_yaml(path: Path) -> dict[str, Any]:
    """Read the simple nested YAML shape used by the F3a config."""
    lines = [
        line.rstrip()
...
def ensure_manual_execution_context(config: dict[str, Any]) -> None:
    ...
    if is_relative_to(script_path, project_root):
        raise RuntimeError(
            "Refusing to execute from the Git-tracked runner. Copy this script to "
            f"{local_copy_root.as_posix()} and change DRY_RUN=False only in that copy."
        )
    if not is_relative_to(script_path, local_copy_root):
        raise RuntimeError(f"Refusing to execute outside the approved local-only runner root: {local_copy_root}")
...
def assert_microbatch_manifest(config: dict[str, Any], rows: list[dict[str, str]]) -> None:
    """Refuse anything other than the requested four-row F3a micro-batch."""
    expected_count = int(config["microbatch"]["expected_run_count"])
    if len(rows) != expected_count:
        raise RuntimeError(f"DO NOT RUN FULL 480. Manifest has {len(rows)} rows, expected {expected_count}.")
    expected_days = set(config["microbatch"]["forcing_days"])
    expected_scenarios = set(config["microbatch"]["scenarios"])
    expected_hour = str(config["microbatch"]["hour_sgt"])
    for row in rows:
        if row.get("forcing_day_id") not in expected_days:
            raise RuntimeError(f"Unexpected forcing day in micro-batch manifest: {row.get('forcing_day_id')}")
        if row.get("scenario") not in expected_scenarios:
            raise RuntimeError(f"Unexpected scenario in micro-batch manifest: {row.get('scenario')}")
        if str(row.get("hour_sgt")) != expected_hour:
            raise RuntimeError(f"Unexpected hour_sgt in micro-batch manifest: {row.get('hour_sgt')}")
...
def scenario_svf_name(config: dict[str, Any], scenario: str) -> str:
    ...
    if scenario == "base":
        return str(config["asset_templates"]["svf_base_zip"])
    if scenario == "overhead_as_canopy":
        return str(config["asset_templates"]["svf_overhead_zip"])
...
def scenario_cdsm_name(config: dict[str, Any], scenario: str) -> str:
    ...
    if scenario == "base":
        return str(config["asset_templates"]["vegetation_base_name"])
    if scenario == "overhead_as_canopy":
        return str(config["asset_templates"]["vegetation_overhead_name"])
    raise ValueError(f"Unknown scenario: {scenario}")


def met_forcing_path(config: dict[str, Any], row: dict[str, str]) -> Path:
    """Return the configured met forcing path for one forcing day."""
    met_paths = config["met_forcing_paths"]
    value = met_paths[row["forcing_day_id"]]
...
def main() -> None:
    """Iterate the four-row F3a manifest and write a local-only run log."""
    config = read_config(CONFIG_PATH)
    manifest_path = repo_path(config["outputs"]["manifest"])
    log_path = Path(str(config["microbatch"]["local_run_log_path"]))
    algorithm_id = str(config["qgis_execution"]["qgis_algorithm_id_hint"])
    rows = read_csv_rows(manifest_path)
    assert_microbatch_manifest(config, rows)
    if not DRY_RUN:
        ensure_manual_execution_context(config)
    print("B8.5-F3a QGIS/SOLWEIG micro-batch runner")
    ...
    print("Complete. Run the F3a postrun validator before any wider execution.")
