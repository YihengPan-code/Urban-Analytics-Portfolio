"""Human-controlled B8.5-F3c QGIS/SOLWEIG N24 / 480-run runner.
...
Default behavior:
    ...
    a human must copy this script to C:/OpenHeat-local/solweig/b85_f3c_n24,
    manually change DRY_RUN to False in that local-only copy, confirm the
    480-row manifest, and run inside QGIS Desktop with UMEP available.
"""
...
CONFIG_PATH = PROJECT_ROOT / "configs/v12/systemb_b85_f3c_n24_full_execution.yaml"
...
def parse_scalar(value: str) -> Any:
    """Parse the small YAML scalar subset used by the F3c config."""
...
def read_simple_yaml(path: Path) -> dict[str, Any]:
    """Read the simple nested YAML shape used by the F3c config."""
    lines = [
        line.rstrip()
...
def assert_n24_manifest(config: dict[str, Any], rows: list[dict[str, str]]) -> None:
    """Refuse anything other than the requested 480-row N24 runset."""
    expected_count = int(config["expected_run_count"])
    expected_cell_count = int(config["expected_cell_count"])
    if len(rows) != expected_count:
        raise RuntimeError(f"N24 / 480 ONLY. Manifest has {len(rows)} rows, expected {expected_count}.")
    unique_cells = {row.get("cell_id", "") for row in rows}
    if len(unique_cells) != expected_cell_count:
        raise RuntimeError(f"N24 / 480 ONLY. Manifest has {len(unique_cells)} cells, expected {expected_cell_count}.")
...
def stop_after_failure(config: dict[str, Any], failures: int, consecutive_failures: int) -> bool:
    ...
    max_failures = int(fail_safe.get("max_failures", 5))
    max_consecutive = int(fail_safe.get("max_consecutive_failures", 3))
    return failures >= max_failures or consecutive_failures >= max_consecutive


def main() -> None:
    """Iterate the 480-row F3c manifest and write a local-only run log."""
    ...
    assert_n24_manifest(config, rows)
    if not DRY_RUN:
        ensure_manual_execution_context(config)
    print("B8.5-F3c QGIS/SOLWEIG N24 / 480 runner")
    print("DO NOT RUN N150.")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if log_row["status"] in {"failed", "blocked"}:
            ...
            consecutive_failures = 0
        print(f"[{idx:03d}/{len(rows):03d}] {row.get('run_id')} {log_row['status']} {log_row['duration_seconds']}s")
        if not DRY_RUN and stop_after_failure(config, failures, consecutive_failures):
            stop_row = base_log_row(
                row,
                now_iso(),
                now_iso(),
                0.0,
            ...
            stop_row["error_message"] = (
                f"Fail-safe stop: failures={failures}, consecutive_failures={consecutive_failures}."
            )
            stop_row["notes"] = "Stopped before any N150/full-AOI expansion; N24 / 480 only."
            write_log_row(log_path, stop_row)
            raise RuntimeError(stop_row["error_message"])
    print("Complete. Run the F3c postrun validator before raster-content QA and stability summaries.")
